...
def get_labels():
    ...
    for cat_id, label in enumerate(class_list):
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    # print(f'test: img1_shape = {img1_shape}, img0_shape = {img0_shape}')
    # print(f'test: gain = {gain}, pad = {pad}')
...
def draw_bbox(bbox, img0, color, wt, names):
    ...
    for idx, class_id in enumerate(bbox[:, 5]):
        ...
        img0 = cv2.putText(img0, str(idx) + ' ' + names[int(class_id)], (int(bbox[idx][0] + wt), int(bbox[idx][1] + wt * 7)), cv2.FONT_HERSHEY_SIMPLEX, wt / 4, (0, 0, 255), max(1, wt // 2))
        img0 = cv2.putText(img0, '{:.4f}'.format(bbox[idx][4]), (int(bbox[idx][0] + wt), int(bbox[idx][1] + wt * 15)), cv2.FONT_HERSHEY_SIMPLEX, wt / 4, (0, 0, 255), max(1, wt // 2))

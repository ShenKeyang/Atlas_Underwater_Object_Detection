...
def output_standardization(output):
    # 转换 [1, 8, 8400] 维度的 ndarray 转化为 [1, 8400, 9] 的形状
...
def scale_coords(img1_shape, coords, img0_shape):
    ...
    # img1_shape = [640, 640], img0_shape = (2160, 3840, 3)
    # gain = 0.16666666666666666, pad = (0.0, 140.0)
...
def draw_bbox(bbox, img0, color, wt, names):
    ...
    for idx, class_id in enumerate(bbox[:, 5]):
        ...
        img0 = cv2.putText(img0, str(idx) + ' ' + names[int(class_id)], (int(bbox[idx][0] + wt), int(bbox[idx][1] + wt * 7)), cv2.FONT_HERSHEY_SIMPLEX, wt / 4, color, max(1, wt // 2))
        img0 = cv2.putText(img0, '{:.4f}'.format(bbox[idx][4]), (int(bbox[idx][0] + wt), int(bbox[idx][1] + wt * 15)), cv2.FONT_HERSHEY_SIMPLEX, wt / 4, color, max(1, wt // 2))
